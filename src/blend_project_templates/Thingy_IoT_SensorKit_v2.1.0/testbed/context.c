...
void sound2ctx(void* sound_in, context_t* context_out) {
  ...
  context_out->value1 = (sound.avg_peak << 16) + sound.max_peak;  
  context_out->value2 = (sound.count_over_thres_per_frame << 16) + sound.avg_all;
  context_out->value3 = sound.avg_over_thres;
  //memcpy(&context_out->value1, &sound.max_peak, sizeof(int16_t));
...
void ctx2sound(context_t* context_in, void** sensor_p) {
  ...
  //memcpy(&sound_p->max_peak, &context_in->value1, sizeof(int16_t));

  sound_p->avg_peak = context_in->value1 >> 16;
  sound_p->max_peak = context_in->value1 & 0xffff;
  sound_p->count_over_thres_per_frame = context_in->value2 >> 16;
  sound_p->avg_all = context_in->value2 & 0xffff;
  sound_p->avg_over_thres = context_in->value3;
