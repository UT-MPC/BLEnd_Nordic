...
void drv_audio_coder_encode(int16_t *raw_samples, m_audio_frame_t *p_frame) {
  ...
  for (int i = 0; i < count_over_thres; ++i) {
    int16_t diff = samples[i] - ((int16_t) avg_over_thres_samples);
    int square = ((int)diff)*((int)diff);
  ...
  int2Bytes(((uint8_t*) p_frame->data) + 4, peak_in_frame);
  int2Bytes(((uint8_t*) p_frame->data) + 8, count_over_thres);
  float2Bytes((uint8_t*) p_frame->data + 12, avg_all_samples);
  float2Bytes((uint8_t*) p_frame->data + 16, avg_over_thres_samples);
