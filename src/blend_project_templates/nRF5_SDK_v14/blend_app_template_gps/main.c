...
//		NRF_LOG_HEXDUMP_INFO(p_data, dlen);
	...
	if (p_blend_evt->evt_id == BLEND_EVT_EPOCH_START)
	{
		...
		NRF_LOG_INFO("Epoch #%d started at time %d\n",epoch_count,time );
	}		
}
    ...
    switch (p_event->evt_type)
    {
        ...
        case APP_UART_DATA_READY:
            ...
            if ((data_array[index - 1] == '\n') || (index >= (50)))
            {
				...
				//char * new_msg = (char*) malloc((index +2) * sizeof(char));
				...
				NRF_LOG_INFO("%s",nrf_log_push(new_msg));
...
int main(void)
{
	...
	//set_blend_data();
	//run_test();
//	bool flag = adafruit_gps_parse("$GPRMC,194509.000,A,4042.6142,N,07400.4168,W,2.03,221.11,160412,,,A*77");
//	adafruit_position_t new_pos = adafruit_get_last_position();
//	NRF_LOG_INFO("lat:" NRF_LOG_FLOAT_MARKER ",  long: "NRF_LOG_FLOAT_MARKER ,NRF_LOG_FLOAT(new_pos.latitude), NRF_LOG_FLOAT(new_pos.longitude));
    for (;;)
    {
